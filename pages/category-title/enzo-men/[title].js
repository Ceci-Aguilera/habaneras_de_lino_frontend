...
export const getStaticProps = async (ctx) => {
  ...
  const response = await axios.get(
    domain + `store/category/title/men/${category_title}/`,
    config
  );
  ...
  return category == undefined ? (
    ...
    <div className={styles.container}>
      <Head>
        ...
        <meta
          property="og:url"
          content={`https://habanerasdelino.com/category-title/enzo-men/${
            category.title
          }`}
        />
      ...
      <main className={styles.main}>
        <h2 className={stylesT.about_title}>
          <span className={stylesT.about_title_span}>{category.title}</span>
        </h2>
